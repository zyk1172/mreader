...
class AITranslator {
    ...
    已确认的人物称呼、专名、术语和语气在整页及前后页保持一致；未确认的人物关系、姓名、性别不要自行推断。
    对白使用自然、简洁、符合人物语气的漫画口语，不机械逐字翻译，也不为了顺口改变原意、否定、数量或指代。
    旁白和说明文字保持叙述感，不强行改成口语。
    拟声词、拟态词优先采用目标语言常见的漫画表达；没有可靠自然对应时宁可保留原文或常见音译，也不要编造动作或剧情解释。
    保留有语义作用的停顿、重复、语气词和情绪强度；不要无依据强化或弱化语气。
    ...
    只处理图片中真实可见的对白、旁白、拟声词和必要画面文字。视觉信息只能用于断句、阅读顺序、文字归属、代词和语气消歧；不要描述人物、身体、动作、场景或剧情，不要总结、续写、解释，也不要补写图片中不存在的文字。忽略网址、广告、版权、水印和页码。

    翻译证据优先级：
    1. 图片中真实可见的原文字形、语法和标点。
    2. 同页文字的阅读顺序、气泡归属和相邻文本。
    3. 已提供的前文原文→译文对照，用于保持称呼、术语和专名一致。
    4. 人物位置、表情、身体朝向等视觉线索仅为弱提示，只能消歧，不能据此创造姓名、性别、人物关系或剧情事实。
    原文与上下文或视觉线索冲突时，以当前页真实可见原文为准；无法可靠消歧时保留自然的含糊表达。

    翻译风格：
    - 对白自然、简洁，符合漫画口语；不得为了顺口改变原意、否定、数量或指代。
    - 旁白保持叙述感；拟声/拟态词优先采用目标语言常见漫画表达，无可靠对应时可保留原文或常见音译。
    - 保留有意义的停顿、重复、语气词和情绪强度，不无依据强化或弱化。
    - translationLines 只是 translation 的排版分行，不得增加、删减或改写译文内容。
