...
struct ContentView: View {
    ...
    private var libraryPage: some View {
        GeometryReader { geometry in
            ...
            let libraryIndex = ShelfLibraryIndex(sortedComics: sortedComicsByTitle(displayComics))
            let seriesItems = shelfFilter == .all
                ? library.series
                : library.series.filter { libraryIndex.visibleSeriesIDs.contains($0.id) }
            let sortedSeriesItems = sortedSeriesByTitle(seriesItems)
            let rootComics = libraryIndex.rootComics
                    ...
                    if shelfDisplayMode == .grid {
                        LazyVGrid(columns: gridLayout.columns, spacing: 24) {
                            ForEach(sortedSeriesItems) { series in
                                seriesGridItem(series, comics: libraryIndex.comics(for: series.id), cardWidth: cardWidth)
                            }
                        ...
                        LazyVStack(spacing: 12) {
                            ForEach(sortedSeriesItems) { series in
                                seriesListItem(series, comics: libraryIndex.comics(for: series.id))
                            }
                            ForEach(rootComics) { comic in
                                comicListItem(comic)
                            }
