...
阅读器手势保持单一职责：双指只负责焦点缩放，放大后单指只平移图片；分页模式由同一个单指拖拽在“翻页”和“下拉关闭”之间判定，未放大时明确向下拖动达到距离阈值才关闭阅读器；连续模式直接使用阅读滚动视图到顶部后的 overscroll，避免退出手势与页面手势竞争。
...
没有可靠气泡边界不代表每条 OCR 行都必须单独翻译。相邻、方向和文字风格连续的 OCR 行会保守地合并成一个 measured paragraph；它仍保持 `bubbleBox = nil`，因此不会凭空制造漫画气泡，最终只产生一个翻译请求和一个 measured-text 卡片。明显分离的行、音效、标牌和其它 standalone 文本仍保持独立。
